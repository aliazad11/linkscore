...
const ANALYSIS_STEPS = [
  { text: "Scanning your profile data...", duration: 12000 },
  { text: "Reading your profile and answers...", duration: 15000 },
  { text: "Mapping your goal to proven strategies...", duration: 15000 },
  { text: "Identifying your content archetype...", duration: 14000 },
  { text: "Generating your post hooks...", duration: 17000 },
  { text: "Building your 30-day roadmap...", duration: 15000 },
  { text: "Calculating your LinkedIn Score...", duration: 12000 },
  { text: "Personalizing your recommendations...", duration: 17000 },
  { text: "Fine-tuning your content strategy...", duration: 16000 },
  { text: "Preparing your final report...", duration: 17000 },
];
...
const GLOBAL_CSS = `
  ...
  *:focus-visible { outline: 2px solid #c8a96e; outline-offset: 2px; border-radius: 4px; }
  ...
  /* Phones: one sticky, swipeable row instead of a 3-deep wrapped pile, so switching
     tabs never requires scrolling back to the top of the report. */
  @media (max-width:640px) {
    .tab-row { flex-wrap:nowrap; overflow-x:auto; position:sticky; top:0; z-index:30; background:#08080e; padding:10px 4px; margin:0 -4px 16px; -webkit-overflow-scrolling:touch; scrollbar-width:none; }
    .tab-row::-webkit-scrollbar { display:none; }
    .tab-row .tab-pill { padding:10px 16px; white-space:nowrap; flex-shrink:0; }
  }
  ...
  /* AI copy can contain long unbroken tokens (URLs, hashtags); break rather than overflow on narrow screens. */
  .card-block p, .week-card p { overflow-wrap:anywhere; }
  .field-input { width:100%; padding:13px 16px; background:#0d0d18; border:1px solid #1a1a2e; border-radius:12px; color:#e8e8f0; font-family:'DM Sans',sans-serif; font-size:16px; outline:none; transition:border-color 0.2s; }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // The gold CTAs are real <a href="?start=1"> links so they work BEFORE hydration
    // (pre-hydration click = navigation, and the ?start=1 check below auto-starts the
    // funnel after load). Once hydrated, preventDefault and start in place instead.
    const start = (e) => { if (e && e.preventDefault) e.preventDefault(); track("funnel_started"); setPhase(cohort ? "form" : "cohort"); };
    const ctas = Array.from(root.querySelectorAll(".btn-gold"));
    ctas.forEach(b => b.addEventListener("click", start));

    // The <details> menus (hamburger + language picker) have no native close-on-
    // navigate in an SPA: close them when a link inside is tapped, and when the
    // user taps anywhere outside the open menu.
    const menus = Array.from(root.querySelectorAll("details.navmenu, details.langpick"));
    const closeMenus = () => menus.forEach(d => d.removeAttribute("open"));
    const menuLinks = Array.from(root.querySelectorAll("details.navmenu a, details.langpick a"));
    menuLinks.forEach(a => a.addEventListener("click", closeMenus));
    const outsideTap = (e) => { menus.forEach(d => { if (d.hasAttribute("open") && !d.contains(e.target)) d.removeAttribute("open"); }); };
    ...
    if (arc && num && gaugeEl) {
      ...
      const runGauge = () => {
        ...
        // SSR markup ships the final value (48) for honesty pre-JS; reset to 0 only at
        // the moment the count-up animation actually begins.
    ...
    return () => {
      ...
      menuLinks.forEach(a => a.removeEventListener("click", closeMenus));
  ...
  // Mobile: every step and quiz-question change starts at the top of the page.
  // Without this, tapping an answer near the bottom leaves the next question
  // scrolled past its heading (the single most disorienting funnel bug on phones).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    run();
    // Honest ~150s pacing means long steps - but a fast generation must not wait for
    // a step boundary. Watch every second and jump the moment the plan is ready.
    const watch = setInterval(() => {
      if (cancelled || !planRef.current) return;
      clearInterval(watch);
      if (planRef.current._error) {
        const msg = planRef.current._error;
        planRef.current = null;
        setGenError(msg);
        setPhase("post_screenshots");
  ...
  const callAPI = async (user, ans, profile, screenshots, cohort=null, specialNote="") => {
    ...
    let reqKey = "";
    try {
      reqKey = localStorage.getItem("ls_reqkey_v1") || "";
      if (!reqKey) {
        reqKey = (window.crypto && window.crypto.randomUUID) ? window.crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2, 12);
        localStorage.setItem("ls_reqkey_v1", reqKey);
      }
    } catch (e) {}
  ...
  const handlePostScreenshots = (index, fileList) => {
    ...
    if (files.length > 1) {
      const empty = [0, 1, 2].filter(i => i !== index && !postScreenshots[i]);
      files.slice(1, 1 + empty.length).forEach((f, k) => handlePostScreenshot(empty[k], f));
    }
  };
      ...
      <div className="page-enter">
        ...
        {acctEmail === undefined ? (
          ...
          <>
            ...
            <a href="https://calendly.com/aliazad1800/how-to-be-a-linkedin-star?utm_source=linkedscore&utm_medium=dashboard&utm_campaign=celebration50" target="_blank" rel="noopener" onClick={()=>track("calendly_clicked", { placement:"dashboard" })} style={{ display: "block", marginTop: 26, background: "linear-gradient(135deg,#16110a,#0d0d18)", border: "1px solid #3a2e16", borderRadius: 16, padding: "20px", textDecoration: "none" }}>
        ...
        {(selected === "Other / Something else" || (q.multiSelect && multiSelected.includes("Other / Something else"))) && (
          <div style={{ marginBottom:16 }}>
            <textarea
              ...
              maxLength={500}
              style={{ width:"100%", minHeight:90, background:"#0d0d18", border:"1px solid #1a1a2e", borderRadius:12, padding:"12px 14px", color:"#F9FAFB", fontSize:16, lineHeight:1.6, resize:"vertical", fontFamily:"inherit", outline:"none", boxSizing:"border-box" }}
            />
      ...
      <div className="page-enter">
        ...
        <textarea
          ...
          style={{
            ...
            padding:"14px 16px", color:"#F9FAFB", fontSize:16,
      ...
      <div className="page-enter">
        ...
        {!noPostsYet && (
          <div style={{ display:"flex", flexDirection:"column", gap:12, marginBottom:20 }}>
            {[0,1,2].map(i => (
              <div key={i}>
                <input ref={postRefs[i]} type="file" accept="image/*" multiple style={{ display:"none" }} onChange={e=>{ handlePostScreenshots(i, e.target.files); e.target.value = ""; }} />
                <div
      ...
      <div className="page-enter" style={{ textAlign:"center" }}>
        ...
        {gateScore != null && (
          <div style={{ display:"flex", flexWrap:"wrap", gap:12, marginBottom:18 }}>
            <div style={{ flex:"1 1 220px", minWidth:0, display:"flex", alignItems:"center", gap:12, background:"#0d0d18", border:"1px solid #1a1a2e", borderRadius:16, padding:14 }}>
    ...
    return (
      <Layout>
        <div className="page-enter" style={{ paddingBottom:40 }}>
          ...
          {/* Scores Row — wraps below ~440px so neither card ever clips off-screen */}
          <div style={{ display:"flex", flexWrap:"wrap", gap:12, marginBottom:20 }}>
            {/* LinkedIn Score */}
            <div style={{ flex:"1 1 220px", minWidth:0, display:"flex", alignItems:"center", gap:14, background:"#0d0d18", border:"1px solid #1a1a2e", borderRadius:16, padding:"16px" }}>
              <ScoreRing score={plan.score} />
              <div>
                <p style={{ color:"#7a7a96", fontSize:9, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:3 }}>{t("sec_linkedin_score")}</p>
                ...
                <p style={{ color:"#f87171", fontSize:12.5, lineHeight:1.45 }}>{plan.urgency}</p>
              </div>
            </div>
            ...
            {plan.thought_leader?.available ? (
              ...
              <div style={{ flex:"1 1 220px", minWidth:0, display:"flex", alignItems:"center", gap:12, background:"#0d0d18", border:"1px dashed #1a1a2e", borderRadius:16, padding:"16px", cursor:"pointer" }} onClick={()=>setPhase("post_screenshots")}>
          ...
          <div className="section-reveal" key={activeSection}>
              ...
              <div>
                {plan.headline_rewrite && (
                  <div className="card-block" style={{ marginBottom:16, borderColor:"#c8a96e33" }}>
                    ...
                    <p style={{ color:"#8a8aa8", fontSize:11.5, lineHeight:1.5, marginTop:10, marginBottom:0 }}>{t("wt_headline")}</p>
                  </div>
                )}
                {plan.about_rewrite && (<div className="card-block" style={{ marginBottom:16, borderColor:"#c8a96e33" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, marginBottom:8 }}><p style={{ color:"#c8a96e", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", margin:0 }}>{t("card_new_about")}</p><CopyBtn text={plan.about_rewrite} /></div><p style={{ color:"#e8e8f0", fontSize:14, lineHeight:1.7, fontWeight:400, whiteSpace:"pre-wrap" }}>{plan.about_rewrite}</p><p style={{ color:"#8a8aa8", fontSize:11.5, lineHeight:1.5, marginTop:10, marginBottom:0 }}>{t("wt_about")}</p></div>)}
                {plan.experience_rewrite && (<div className="card-block" style={{ marginBottom:16, borderColor:"#c8a96e33" }}><div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, marginBottom:8 }}><p style={{ color:"#c8a96e", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", margin:0 }}>{t("card_new_exp")}</p><CopyBtn text={plan.experience_rewrite} /></div><p style={{ color:"#e8e8f0", fontSize:14, lineHeight:1.7, fontWeight:400, whiteSpace:"pre-wrap" }}>{plan.experience_rewrite}</p><p style={{ color:"#8a8aa8", fontSize:11.5, lineHeight:1.5, marginTop:10, marginBottom:0 }}>{t("wt_exp")}</p></div>)}
            ...
            {activeSection===3 && (
              <div>
                {plan.ssi_plan?.available ? (
                  <>
                    <p style={{ color:"#7a7a96", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:6 }}>{t("ssi_overview")}</p>
                    <p style={{ color:"#8a8aa8", fontSize:11.5, lineHeight:1.5, marginTop:0, marginBottom:10 }}>{t("wt_ssi")}</p>
            ...
            {activeSection===5 && (
              <div>
                <p style={{ color:"#7a7a96", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:8 }}>{t("hooks_title")}</p>
                <p style={{ color:"#8a8aa8", fontSize:11.5, lineHeight:1.5, marginTop:0, marginBottom:14 }}>{t("wt_hooks")}</p>
            ...
            {activeSection===6 && (
              <div>
                <p style={{ color:"#7a7a96", fontSize:10, fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:8 }}>{t("roadmap_title")}</p>
                <p style={{ color:"#8a8aa8", fontSize:11.5, lineHeight:1.5, marginTop:0, marginBottom:14 }}>{t("wt_calendar")}</p>
          ...
          {/* Share card (banner + ready-to-paste caption) shows on the owner's result page
              ...
              fallback when no card exists) stays owner-only. Lives BELOW the report so the
              score and diagnosis, not a share prompt, are what a phone user sees first. */}
