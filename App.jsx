...
function finalizePlan(plan, rev, locale = "en", hadProfile = true) {
  ...
  if (plan.content_strategy && typeof plan.content_strategy === "object") {
    ...
    plan.content_strategy.content_types = fixCadence(plan.content_strategy.content_types);
  }
  if (Array.isArray(plan.content_calendar)) plan.content_calendar = plan.content_calendar.map((w) => (w && typeof w === "object") ? { ...w, topic: fixCadence(w.topic), action: fixCadence(w.action) } : w);
  if (Array.isArray(plan.growth_tactics)) plan.growth_tactics = plan.growth_tactics.map(fixCadence);
