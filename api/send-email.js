...
export default async function handler(req) {
  ...
  try {
    ...
    const hooks = plan.post_hooks?.map((h) =>
      `<li style="margin-bottom:12px;padding:14px;background:#0d0d18;border:1px solid #1a1a2e;border-left:3px solid #c8a96e;border-radius:8px;color:#e8e8f0;font-size:14px;line-height:1.6;list-style:none;">${h}</li>`
    ).join('') || '';
    ...
    const thoughtLeaderBlock = plan.thought_leader?.available ? `
    <div style="background:#0d0d18;border:1px solid #1a1a2e;border-radius:16px;padding:24px;margin-bottom:16px;">
      ...
      <p style="color:#c8a96e;font-size:52px;font-weight:800;margin:0 0 6px;">${plan.thought_leader.score}<span style="font-size:18px;color:#3a3a5a;">/100</span></p>
      <div style="margin-top:14px;">
        ${[['Hook Quality', plan.thought_leader.hook_score], ['Engagement', plan.thought_leader.engagement_score], ['Voice', plan.thought_leader.voice_score], ['Structure', plan.thought_leader.structure_score]].map(([label, score]) => `
      ...
      <p style="color:#4a4a6a;font-size:13px;line-height:1.6;margin-top:12px;padding-top:12px;border-top:1px solid #1a1a2e;">${plan.thought_leader.analysis}</p>
    </div>` : '';
...
<body style="margin:0;padding:0;background:#08080e;font-family:'Segoe UI',sans-serif;">
  <div style="max-width:580px;margin:0 auto;padding:40px 20px;">
    ...
    <p style="color:#4a4a6a;font-size:14px;line-height:1.7;margin-bottom:32px;">${plan.headline}</p>
    <div style="background:#0d0d18;border:1px solid #1a1a2e;border-radius:16px;padding:24px;margin-bottom:16px;">
